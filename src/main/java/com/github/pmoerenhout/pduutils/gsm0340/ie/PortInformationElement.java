...
{
	public static final int IEI_PORT_16BIT = 0x05;

	PortInformationElement(byte id, byte[] data)
	{
		super(id, data);
		if (getIdentifier() != IEI_PORT_16BIT) { throw new RuntimeException("Invalid identifier " + getIdentifier() + " in data in: " + getClass().getSimpleName()); }
		// iei
		// iel
		// dest(2 bytes)
		// src (2 bytes)
