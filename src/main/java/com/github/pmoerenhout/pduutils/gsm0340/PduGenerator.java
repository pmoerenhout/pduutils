...
public class PduGenerator {
  ...
  protected void writeTimeStampStringForDate(ZonedDateTime timestamp) {
    ...
    ZoneOffset zoneOffset = timestamp.getOffset();
    int tzValue = zoneOffset.getTotalSeconds() / 900;
  ...
  protected void checkForConcat(Pdu pdu, int lengthOfText, int maxLength, int maxLengthWithUdh, int mpRefNo, int partNo) {
    if ((lengthOfText <= maxLengthWithUdh) || ((lengthOfText > maxLengthWithUdh) && (lengthOfText <= maxLength))) {
      ...
      if (pdu.getConcatInfo() != null) {
        ...
        // the UDH length
        ConcatenationInformationElement concatInfo = InformationElementFactory.generateConcatenationInformationElement(mpRefNo, 255, partNo);
  ...
  protected int computePotentialUdhLength(Pdu pdu) {
    ...
    if (currentUdhLength == 0) {
      ...
      return InformationElementFactory.getDefaultConcatenationLength() + 1;
    }
    // this already has the UDH Length field, no need to add 1
    return currentUdhLength + InformationElementFactory.getDefaultConcatenationLength();
  }
  ...
  protected void generateSmsDeliverPduString(SmsDeliveryPdu pdu, int mpRefNo, int partNo) throws Exception {
    ...
    writeTimeStampStringForDate(pdu.getServiceCentreTimestamp());
    // user data
    // headers
    writeUDData(pdu, mpRefNo, partNo);
  }
