...
public class PduUtils {
  ...
  // TP-RP    xxxx0xxx = reply path (for SMS-DELIVER only)
  //          xxxx1xxx = no reply path
  public static final int TP_RP_MASK = 0xF7;
  public static final int TP_RP_NO_REPLY_PATH = 0x00;
  public static final int TP_RP_REPLY_PATH = 0x08;
  ...
  private static final char[] GSM_ALPHABET = { '\u0040', // COMMERCIAL AT
      '\u00A3', // POUND SIGN
      '\u0024', // DOLLAR SIGN
      '\u00A5', // YEN SIGN
  ...
  public static byte swapNibbles(final int b) {
  ...
  public static byte[] unencodedSeptetsToEncodedSeptets(final byte[] septetBytes) {
    byte[] txtBytes;
    byte[] txtSeptets;
    int txtBytesLen;
    BitSet bits;
  ...
  public static int getNumMultiCharsInSeptets(final byte[] bytes) {
    int count = 0;
  ...
  public static String unencodedSeptetsToString(final byte[] bytes) {
    ...
    for (i = 0; i < bytes.length; i++) {
      if (bytes[i] == 0x1b) {
        ...
        text.append(GSM_ALPHABET[bytes[i]]);
      }
    }
    return text.toString();
  }

  public static String encodedSeptetsToString(final byte[] encodedSeptets) {
    return unencodedSeptetsToString(encodedSeptetsToUnencodedSeptets(encodedSeptets));
  }

  public static int getNumSeptetsForOctets(final int numOctets) {
    return numOctets * 8 / 7 + ((numOctets * 8 % 7 != 0) ? 1 : 0);
    //return numOctets + (numOctets/7);
  }

  // decompress encoded septets to unencoded form
  public static byte[] encodedSeptetsToUnencodedSeptets(final byte[] octetBytes) {
    return encodedSeptetsToUnencodedSeptets(octetBytes, true);
  }

  public static byte[] encodedSeptetsToUnencodedSeptets(final byte[] octetBytes, final boolean discardLast) {
    byte newBytes[];
    BitSet bitSet;
  ...
  public static byte[] pduToBytes(final String s) {
  ...
  public static String pduToBits(final String pduString) {
    return bytesToBits(pduToBytes(pduString));
  }

  // converts a byte array to PDU style string
  public static String bytesToPdu(final byte[] bytes) {
    StringBuilder sb = new StringBuilder();
    for (int i = 0; i < bytes.length; i++) {
  ...
  public static String bytesToBits(final byte[] b) {
    StringBuilder sb = new StringBuilder();
  ...
  public static String byteToBits(final byte b) {
  ...
  public static String byteToPdu(final int b) {
