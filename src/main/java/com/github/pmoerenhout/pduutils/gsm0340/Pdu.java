...
public abstract class Pdu {
  ...
  protected int getFirstOctetField(final int fieldName) {
  ...
  public void setAddress(MsIsdn address) {
    if (address.getType() == MsIsdn.Type.VOID) {
      this.address = "";
    } else {
  ...
  public int getTotalUDHLength() {
    ...
    // also takes into account the field holding the length itself
    return udhLength + 1;
  }
  ...
  public ConcatenationInformationElement getConcatInfo() {
    checkForUDHI(UDH_CHECK_MODE_IGNORE_IF_NONE);
    ConcatenationInformationElement concat = (ConcatenationInformationElement) getInformationElement(Concatenation8InformationElement.IEI_CONCATENATION_8BIT);
    if (concat == null) {
      concat = (ConcatenationInformationElement) getInformationElement(Concatenation16InformationElement.IEI_CONCATENATION_16BIT);
    }
    return concat;
  }

  public int getMpRefNo() {
    ConcatenationInformationElement concat = getConcatInfo();
    if (concat != null) {
  ...
  private PortInformationElement getPortInfo() {
    checkForUDHI(UDH_CHECK_MODE_IGNORE_IF_NONE);
    return (PortInformationElement) getInformationElement(PortInformationElement.IEI_PORT_16BIT);
  }
  ...
  protected String formatTimestamp(final ZonedDateTime timestamp) {
